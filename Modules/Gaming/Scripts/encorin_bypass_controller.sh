...
enable_bypass() {
    ...
    if [ "$supported" != "Yes" ]; then
        ...
        echo "Run '$0 test' first to check support"
        return 1
    fi
    ...
    if [ -z "$methods" ]; then
        echo "No supported methods found. Run '$0 test' first"
        return 1
    fi
    
    # Try each supported method until one works
    for method in $methods; do
        echo "Trying to enable bypass using method: $method"
        if apply_bypass_method "$method" "bypass"; then
            echo "✓ Bypass enabled successfully using method: $method"
            update_config "BYPASS" "Yes"
            update_config "ACTIVE_METHOD" "$method"
            return 0
        else
            echo "✗ Failed to enable bypass using method: $method"
        fi
    done
    
    echo "Failed to enable bypass charging with any supported method"
...
disable_bypass() {
    ...
    if [ -z "$method" ]; then
        echo "No active bypass method found"
        # Try to restore all supported methods just in case
        local methods=$(get_config "SUPPORTED_METHODS")
        if [ -n "$methods" ]; then
            echo "Attempting to restore all supported methods..."
            for m in $methods; do
        ...
        if apply_bypass_method "$method" "restore"; then
            echo "✓ Normal charging restored successfully"
        else
            echo "✗ Failed to restore normal charging"
        fi
    fi
...
check_status() {
    ...
    echo "Active Method: $(get_config "ACTIVE_METHOD")"
    echo "Supported Methods: $(get_config "SUPPORTED_METHODS")"
    echo "Default Method: $(get_config "DEFAULT_METHOD")"
    echo ""
    
    # Show current values of charging nodes if bypass is active
    local active_method=$(get_config "ACTIVE_METHOD")
    if [ -n "$active_method" ] && [ "$(get_config "BYPASS")" = "Yes" ]; then
        echo "=== Current Node Values ==="
        case "$active_method" in
            "OPLUS_MMI")
                for node in "${OPLUS_MMI_NODES[@]}"; do
                    if [ -r "$node" ]; then
                        echo "$node: $(read_node "$node")"
                    fi
                done
                ;;
            "SUSPEND_COMMON")
...
# Function to show detailed info about available methods
show_methods() {
    echo "=== Available Bypass Methods ==="
    echo ""
    echo "OPLUS_MMI - OnePlus MMI charging control"
    echo "SUSPEND_COMMON - Common input suspend nodes"
    echo "DISABLE_COMMON - Common charge disable nodes"
    echo "GOOGLE_PIXEL - Google Pixel specific nodes"
    echo "SAMSUNG_STORE_MODE - Samsung store mode"
    echo "QCOM_SUSPEND - Qualcomm suspend control"
    echo "HUAWEI_COMMON - Huawei charging control"
    echo "CONTROL_COMMON - Common charger control"
    echo "TRANSISSION_BYPASSCHG - Transission bypass"
    echo ""
    echo "Use '$0 test' to check which methods are supported on your device"
    echo "==============================="
}
...
show_usage() {
    ...
    echo "  status   - Show current bypass status and configuration"
    echo "  methods  - Show information about available bypass methods"
